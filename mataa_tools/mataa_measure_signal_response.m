...
% X0_unit (optional): unit of test signal data in X0 (string). Default: X0_unit = 'digital'.
%	the unit string can be one of the following:
%	unit = 'digital': X0 signal is given in digital domain. The X0 values are sent to the DAC without any amplitude conversion. X0 values are allowed to range from -1 to +1, corresponding to the min. and max. value of the analog signal at the DAC output.
%	unit = 'V' or 'Volt': X0 signal is given in volts; X0 reflects the signal voltage that is generated at the DAC output. The X0 voltages are converted to "digital domain values" using the DAC sensitivity given in the 'cal' data before the data is sent the DAC. X0 values are allowed to range from the min. to max. voltages that can be generated by the DAC output.
% 
% OUTPUT:
...
% > fs = 44100;
% > [s,t] = mataa_signal_generator ('sine',fs,0.2,1000);
% > s = 1.8 * s; % set zero-to-peak amplitude to 1.8 (Volt)
% > [out,in,t,out_unit,in_unit] = mataa_measure_signal_response(s,fs,0.1,1,1,'GENERIC_CHAIN_DIRECT.txt','Volt');
...
switch toupper (X0_unit)

	case {'','DIGITAL'} % no amplitude conversion
		if max(X0) > 1.0
	...
	case {'V','VOLT'} % convert data values to digital before sending to DAC
		% convert X0 voltage to "digital domain"
		if ~isfield(cal,'DAC')
			error ('mataa_measure_signal_response: cal data has no DAC field, cannot convert X0 voltages to digital domain.')
		else
			if ~strcmp(cal.DAC.sensitivity_unit,'V')
				error (sprintf('mataa_measure_signal_response: X0 data given in Volts, but cal data has different units for DAC output (%s).',cal.DAC.sensitivity_unit))
			else
				if max(X0) > cal.DAC.sensitivity
					error (sprintf('mataa_measure_signal_response: max. value of X0 (%g Volts) is higher than analog output limit of DAC (%g Volts)',max(X0),cal.DAC.sensitivity))
				elseif min(X0) < -cal.DAC.sensitivity
					error (sprintf('mataa_measure_signal_response: min. value of X0 (%g Volts) is lower than analog output limit of DAC (%g Volts)',min(X0),cal.DAC.sensitivity))
				end
	...
	otherwise
		error (sprintf('mataa_measure_signal_response: X0 unit %s unknown.','X0_unit'))	
